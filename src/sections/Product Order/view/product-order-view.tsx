...
import { ProductOrderTableHead } from '../product-order-table-head';
import { TableEmptyRows } from '../product-order-table-empty-rows';
import { ProductOrderTableToolbar } from '../product-order-table-toolbar';
import { emptyRows, applyFilter, getComparator } from '../utils';

import type { ProductOrderProps } from '../product-order-table-row';
import {ProductOrderModel} from "../../../models/ProductOrder";
import {getAllProductOrders} from "../../services/shopService";
...
export function ProductOrderView() {
  ...
  const [dataFiltered, setDataFiltered] = useState<ProductOrderProps[]>();
  const [modifiedId, setModifiedId] = useState<number>(-1);
  const [_users, setUsers] = useState<ProductOrderModel[]>([]);
...
const schema = z.object({
  ...
  const loadData = useCallback(async () => {
    const memberships = await getAllProductOrders();
    console.log(memberships);

    if (memberships.status) {
      setUsers(memberships.data!);
      setDataFiltered(applyFilter({
        inputData: memberships.data!.map(m => m.toProductOrderProps()),
