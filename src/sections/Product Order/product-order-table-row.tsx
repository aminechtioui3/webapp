...
import {deleteProductOrder} from "../services/shopService";
...
export function ProductOrderTableRow({ row, selected, onSelectRow, updateData, onDeleteSuccess  }: MembershipTableRowProps) {
  ...
  const deleteM = useCallback(async () => {
    const result = await deleteProductOrder(row.id.toString());
    if (result.status) {
      onDeleteSuccess();
