...
import {deleteGymFacility} from "../services/GymService";


// ----------------------------------------------------------------------

export type GymFacilitiesProps = {
  id: number;
  name: string;
};

type GymFacilitiesTableRowProps = {
  row: GymFacilitiesProps;
  selected: boolean;
  onSelectRow: () => void;
  updateData: any;
...
export function GymFacilitiesTableRow({ row, selected, onSelectRow, updateData, onDeleteSuccess  }: GymFacilitiesTableRowProps) {
  ...
  const deleteM = useCallback(async () => {
    const result = await deleteGymFacility(row.id.toString());
    if (result.status) {
      onDeleteSuccess();
