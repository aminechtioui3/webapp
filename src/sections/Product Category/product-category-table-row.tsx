...
import {deleteProductCategory} from "../services/shopService";
...
export function ProductCategoryTableRow({ row, selected, onSelectRow, updateData, onDeleteSuccess  }: MembershipTableRowProps) {
  ...
  const deleteM = useCallback(async () => {
    const result = await deleteProductCategory(row.id.toString());
    if (result.status) {
      onDeleteSuccess();
